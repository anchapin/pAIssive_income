...
def create_research_team(user_id: str) -> MemoryEnhancedCrewAIAgentTeam:
    """
    Creates a research team with memory-enhanced agents.
...
def run_example() -> None:
    """Runs the example workflow with memory-enhanced agents."""
    # Check dependencies
    ...
    if team.memory is not None:
        ...
        try:
            ...
            if memories and isinstance(memories, dict) and "results" in memories:
                ...
                logger.info("Retrieved %d memories:", len(memory_results))
                for i, memory in enumerate(memory_results):
                    memory_text = memory.get("text", "No text")
                    logger.info("Memory %d: %s...", i + 1, memory_text[:100])
            else:
                logger.info("No memories retrieved or unexpected format")
        except Exception:
