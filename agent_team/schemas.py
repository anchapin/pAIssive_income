...
class WorkflowSettingsSchema(BaseModel):
    ...
    auto_progression: bool = Field(
        False, 
        description="Whether to automatically progress through workflow steps"
    )
...
class NicheSchema(BaseModel):
    ...
    target_audience: Optional[List[str]] = Field(
        None, 
        description="Target audience for the niche"
    )
...
class MonetizationStrategySchema(BaseModel):
    ...
    pricing_tiers: List[PricingTierSchema] = Field(
        ..., 
        description="Pricing tiers"
    )
    target_revenue: Optional[float] = Field(
        None, 
...
class MarketingPlanSchema(BaseModel):
    ...
    launch_plan: Optional[Dict[str, Any]] = Field(
        None, 
