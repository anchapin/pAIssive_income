...
def test_pricing_calculator():
    ...
    assert prices[free_tier["id"]] == 0.0, f"Expected free tier price to be 0.0, got {prices[free_tier['id']]}"
    assert prices[basic_tier["id"]] == 9.99, f"Expected basic tier price to be 9.99, got {prices[basic_tier['id']]}"
    assert prices[pro_tier["id"]] == 19.99, f"Expected pro tier price to be 19.99, got {prices[pro_tier['id']]}"
    assert prices[premium_tier["id"]] == 49.99, f"Expected premium tier price to be 49.99, got {prices[premium_tier['id']]}"
    ...
    assert len(analysis["price_points"]) == 5, f"Expected 5 price points, got {len(analysis['price_points'])}"
    assert analysis["base_price"] == 19.99, f"Expected base price to be 19.99, got {analysis['base_price']}"
    assert analysis["market_size"] == 10000, f"Expected market size to be 10000, got {analysis['market_size']}"

    # Test to_dict and to_json
    calculator_dict = calculator.to_dict()
    assert calculator_dict["name"] == "Test Pricing Calculator", f"Expected 'Test Pricing Calculator', got '{calculator_dict['name']}'"
    ...
    assert loaded_calculator.name == calculator.name, f"Expected '{calculator.name}', got '{loaded_calculator.name}'"
    assert loaded_calculator.pricing_strategy == calculator.pricing_strategy, f"Expected '{calculator.pricing_strategy}', got '{loaded_calculator.pricing_strategy}'"
