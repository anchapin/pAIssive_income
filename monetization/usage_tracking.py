...
class UsageLimit:
    ...
    def get_period_end(self, reference_time: Optional[datetime] = None) -> datetime:
        ...
        if self.period == self.PERIOD_HOURLY:
            return self.get_period_start(reference_time) + timedelta(hours=1) - timedelta(microseconds=1)
        elif self.period == self.PERIOD_DAILY:
            return self.get_period_start(reference_time) + timedelta(days=1) - timedelta(microseconds=1)
        elif self.period == self.PERIOD_WEEKLY:
            return self.get_period_start(reference_time) + timedelta(days=7) - timedelta(microseconds=1)
        elif self.period == self.PERIOD_MONTHLY:
    ...
    def is_within_period(self, timestamp: datetime, reference_time: Optional[datetime] = None) -> bool:
        """
