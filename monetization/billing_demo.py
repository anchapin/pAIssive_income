...
from .usage_tracking import UsageMetric, UsageCategory, UsageRecord, UsageLimit, UsageQuota
from .usage_tracker import UsageTracker
from .billing_calculator import BillingCalculator, PricingModel, PricingTier, PricingRule
...
def run_demo():
    ...
    for item in usage_cost["items"]:
        print(f"- {item['metric']} ({item['category']}, {item['resource_type']}): {item['quantity']} units, ${item['cost']:.2f}")
    
    print_separator()
    
    # Get detailed cost breakdown for tokens
    token_usage = sum(item["quantity"] for item in usage_cost["items"] if item["metric"] == UsageMetric.TOKEN and item["category"] == UsageCategory.INFERENCE)
    ...
    for tier in breakdown["tiers"]:
        ...
        print(f"- {tier['min_quantity']}-{max_str}: {tier['quantity']} units at ${tier['price_per_unit']}/unit = ${tier['cost']:.2f}")
    
    print(f"\nSubtotal: ${breakdown['subtotal']:.2f}")
    
    if breakdown["volume_discount"]:
        discount = breakdown["volume_discount"]
        print(f"Volume discount: {discount['discount_percentage']}% off for {discount['min_quantity']}+ units = -${discount['discount_amount']:.2f}")
