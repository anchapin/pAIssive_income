...
def test_freemium_model():
    ...
    model = FreemiumModel(
        ...
        description="A test freemium subscription model"
    )
    
    # Verify that the free tier was created
    assert len(model.tiers) == 1, f"Expected 1 tier (free tier), got {len(model.tiers)}"
    assert model.tiers[0]["price_monthly"] == 0.0, f"Expected free tier price to be 0.0, got {model.tiers[0]['price_monthly']}"
    ...
    assert len(model.tiers) == 3, f"Expected 3 tiers (free + 2 paid), got {len(model.tiers)}"
    ...
    assert len(free_features) == 1, f"Expected 1 feature in free tier, got {len(free_features)}"
    assert free_features[0]["name"] == "Feature 1", f"Expected 'Feature 1', got '{free_features[0]['name']}'"
    ...
    assert free_tier["limits"]["api_calls"] == 50, f"Expected api_calls limit to be 50, got {free_tier['limits']['api_calls']}"
    
    # Test to_dict
    model_dict = model.to_dict()
    assert model_dict["model_type"] == "freemium", f"Expected model_type to be 'freemium', got '{model_dict.get('model_type')}'"
    assert model_dict["free_tier_id"] == free_tier_id, f"Expected free_tier_id to be '{free_tier_id}', got '{model_dict.get('free_tier_id')}'"
    ...
    assert len(loaded_model.tiers) == len(model.tiers), f"Expected {len(model.tiers)} tiers, got {len(loaded_model.tiers)}"
