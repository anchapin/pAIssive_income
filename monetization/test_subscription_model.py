...
def test_subscription_model():
    ...
    assert tier1_features[0]["name"] == "Feature 1", f"Expected 'Feature 1', got '{tier1_features[0]['name']}'"
    ...
    assert model.tiers[0]["price_monthly"] == 14.99, f"Expected price 14.99, got {model.tiers[0]['price_monthly']}"
    
    # Test to_dict and to_json
    model_dict = model.to_dict()
    assert model_dict["name"] == "Test Subscription Model", f"Expected 'Test Subscription Model', got '{model_dict['name']}'"
    ...
    assert len(loaded_model.features) == len(model.features), f"Expected {len(model.features)} features, got {len(loaded_model.features)}"
