...
class InvoiceFormatter:
    ...
    def to_html(invoice: Invoice) -> str:
        ...
        html.append("body { font - family: Arial, 
            sans - serif; margin: 0; padding: 20px; }")
        html.append(
            ".invoice { max - \
                width: 800px; margin: 0 auto; border: 1px solid #ccc; padding: 20px; }"
        )
        html.append(
            ".header { display: flex; justify - \
        ...
        html.append(".items { width: 100%; border - \
            collapse: collapse; margin - bottom: 20px; }")
        html.append(".items th, .items td { padding: 8px; text - align: right; }")
        html.append(".items th:first - child, 
        ...
        html.append(
            ".status { display: inline - \
                block; padding: 5px 10px; border - radius: 3px; font - weight: bold; }"
        )
        ...
        html.append(".status - \
            partially_paid { background - color: #e8f5e9; color: #7cb342; }")
        html.append(".status - \
            overdue { background - color: #ffebee; color: #d32f2f; }")
        html.append(".status - \
            canceled { background - color: #f0f0f0; color: #757575; }")
        ...
        html.append(
            f'<div><strong>Due Date:</strong> {invoice.due_date.strftime(
                " % Y-%m-%d")}</div>'
        )
        ...
        html.append(
            f'<div style="margin - \
                top: 10px;"><span class="{status_class}">{invoice.status.upper()}</span></div>'
        )
        ...
        html.append(
            f"<tr><td>Subtotal:</td><td>{invoice.format_amount(
                invoice.get_subtotal())}</td></tr>"
        )

        if invoice.get_discount_total() > 0:
            html.append(
                f"<tr><td>{invoice.discount_name}:</td><td>{invoice.format_amount(
                    invoice.get_discount_total())}</td></tr>"
            )

        if invoice.get_tax_total() > 0:
            html.append(
                f"<tr><td>{invoice.tax_name}:</td><td>{invoice.format_amount(
                    invoice.get_tax_total())}</td></tr>"
            )
        ...
        for fee in invoice.additional_fees:
            ...
            html.append(
                f"<tr><td>{fee_name}:</td><td>{invoice.format_amount(
                    fee_amount)}</td></tr>"
            )

        html.append(
            f'<tr class="total - \
                row"><td>Total:</td><td>{invoice.format_amount(invoice.get_total())}</td></tr>'
        )
        ...
        if invoice.payments:
            ...
            for payment in invoice.payments:
                payment_date = \
                    datetime.fromisoformat(payment["date"]).strftime(" % Y-%m-%d")
                html.append(
                    f'<tr><td>{payment_date} - \
                        {payment["payment_method"]}:</td><td>-{invoice.format_amount(payment["amount"])}</td></tr>'
                )

            html.append(
                f'<tr class="total - \
                    row"><td>Balance Due:</td><td>{invoice.format_amount(invoice.get_balance_due())}</td></tr>'
            )
    ...
    def to_text(invoice: Invoice) -> str:
        ...
        if invoice.get_discount_total() > 0:
            lines.append(
                f"{invoice.discount_name + \
                    's:':<65} {invoice.format_amount(invoice.get_discount_total()):>15}"
            )

        if invoice.get_tax_total() > 0:
            lines.append(
                f"{invoice.tax_name + \
                    'es:':<65} {invoice.format_amount(invoice.get_tax_total()):>15}"
            )
        ...
        for fee in invoice.additional_fees:
            ...
            lines.append(f"{fee_name + \
                's:':<65} {invoice.format_amount(fee_amount):>15}")
        ...
        if invoice.payments:
            ...
            for payment in invoice.payments:
                ...
                lines.append(
                    f"{payment_date} - \
                        {payment['payment_method']}:{' ' * 30} -{invoice.format_amount(payment['amount']):>15}"
                )

            lines.append(" - " * 80)
            lines.append(
                f"{'Balance Due:':<65} {invoice.format_amount(
                    invoice.get_balance_due()):>15}"
            )
    ...
    def to_csv(invoice: Invoice) -> str:
        ...
        # Add header
        lines.append(
            "Invoice Number,Date,Due Date,Status,Customer ID,Customer Name,Total,
                Balance Due"
        )
        ...
        # Add items header
        lines.append("Description,Quantity,Unit Price,Discount,Tax Rate,Tax Amount,
            Total")
...
class InvoiceDelivery:
    ...
    def send_invoice_by_email(
        ...
        Args:
            ...
            email: Override recipient email address (
                uses customer email from invoice if not provided)
            subject: Custom email subject line (auto - \
                generated from invoice details if not provided)
            message: Custom email message body (auto - \
                generated with payment details if not provided)
        ...
        if not message:
            message = \
                f"Please find attached invoice {invoice.number} for {invoice.format_amount(invoice.get_total())}."

            if invoice.due_date:
                message += f" Payment is due by {invoice.due_date.strftime(
                    ' % Y-%m-%d')}."
    ...
    def export_invoice(
        ...
        """
        Convert invoice to multiple export formats through format - \
            specific serialization pipeline.

        This algorithm implements a comprehensive multi - \
            format invoice export system with
        dynamic content adaptation and format - \
            specific optimization. The implementation
        follows these key phases:
        ...
        Args:
            ...
            format: Target export format - one of "json", "html", "text", "csv", 
                or "pdf"
            output_path: Optional file path to save the exported data (
                returns data in memory if not provided)
    ...
    def export_invoices(
        ...
        This algorithm implements a high - \
            performance batch processing system for invoice
...
if __name__ == "__main__":
    ...
    text_output = delivery.export_invoice(invoice, format="text", 
        output_path="invoice_example.txt")
    csv_output = delivery.export_invoice(invoice, format="csv", 
        output_path="invoice_example.csv")
