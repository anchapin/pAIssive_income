...
class InvoiceFormatter:
    ...
    def to_html(invoice: Invoice) -> str:
        ...
        html.append(".invoice { max-width: 800px; margin: 0 auto; border: 1px solid #ccc; padding: 20px; }")
        ...
        html.append(".invoice-title { font-size: 24px; font-weight: bold; color: #333; margin-bottom: 5px; }")
        ...
        html.append(".status { display: inline-block; padding: 5px 10px; border-radius: 3px; font-weight: bold; }")
        ...
        if invoice.company_logo_url:
            html.append(f'<img src="{invoice.company_logo_url}" alt="{invoice.company_name}" style="max-height: 100px; margin-bottom: 10px;">')
        ...
        html.append(f'<div><strong>Due Date:</strong> {invoice.due_date.strftime("%Y-%m-%d")}</div>')
        
        status_class = f"status status-{invoice.status.lower()}"
        html.append(f'<div style="margin-top: 10px;"><span class="{status_class}">{invoice.status.upper()}</span></div>')
        ...
        html.append(f'<tr><td>Subtotal:</td><td>{invoice.format_amount(invoice.get_subtotal())}</td></tr>')
        
        if invoice.get_discount_total() > 0:
            html.append(f'<tr><td>{invoice.discount_name}:</td><td>{invoice.format_amount(invoice.get_discount_total())}</td></tr>')
        
        if invoice.get_tax_total() > 0:
            html.append(f'<tr><td>{invoice.tax_name}:</td><td>{invoice.format_amount(invoice.get_tax_total())}</td></tr>')
        ...
        for fee in invoice.additional_fees:
            ...
            html.append(f'<tr><td>{fee_name}:</td><td>{invoice.format_amount(fee_amount)}</td></tr>')
        
        html.append(f'<tr class="total-row"><td>Total:</td><td>{invoice.format_amount(invoice.get_total())}</td></tr>')
        ...
        if invoice.payments:
            ...
            for payment in invoice.payments:
                ...
                html.append(f'<tr><td>{payment_date} - {payment["payment_method"]}:</td><td>-{invoice.format_amount(payment["amount"])}</td></tr>')
            
            html.append(f'<tr class="total-row"><td>Balance Due:</td><td>{invoice.format_amount(invoice.get_balance_due())}</td></tr>')
    ...
    def to_text(invoice: Invoice) -> str:
        ...
        lines.append("Items:")
        lines.append("-" * 80)
        lines.append(f"{'Description':<40} {'Quantity':>10} {'Unit Price':>15} {'Tax':>15} {'Total':>15}")
        lines.append("-" * 80)
        ...
        if invoice.get_discount_total() > 0:
            lines.append(f"{invoice.discount_name+'s:':<65} {invoice.format_amount(invoice.get_discount_total()):>15}")
        
        if invoice.get_tax_total() > 0:
            lines.append(f"{invoice.tax_name+'es:':<65} {invoice.format_amount(invoice.get_tax_total()):>15}")
        ...
        if invoice.payments:
            ...
            for payment in invoice.payments:
                payment_date = datetime.fromisoformat(payment["date"]).strftime("%Y-%m-%d")
                lines.append(f"{payment_date} - {payment['payment_method']}:{' ' * 30} -{invoice.format_amount(payment['amount']):>15}")
            
            lines.append("-" * 80)
            lines.append(f"{'Balance Due:':<65} {invoice.format_amount(invoice.get_balance_due()):>15}")
    ...
    def to_csv(invoice: Invoice) -> str:
        ...
        # Add header
        lines.append("Invoice Number,Date,Due Date,Status,Customer ID,Customer Name,Total,Balance Due")
...
class InvoiceDelivery:
    ...
    def export_invoices(
        ...
        if format == "csv":
            # Generate CSV header
            lines = ["Invoice Number,Date,Due Date,Status,Customer ID,Customer Name,Total,Balance Due"]
...
if __name__ == "__main__":
    ...
    html_output = delivery.export_invoice(invoice, format="html", output_path="invoice_example.html")
