...
class Subscription:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Subscription':
        ...
        subscription.canceled_at = datetime.fromisoformat(data["canceled_at"]) if data.get("canceled_at") else None
        subscription.trial_end = datetime.fromisoformat(data["trial_end"]) if data.get("trial_end") else None
        subscription.current_period_start = datetime.fromisoformat(data["current_period_start"])
        subscription.current_period_end = datetime.fromisoformat(data["current_period_end"]) if data.get("current_period_end") else None
        subscription.next_billing_date = datetime.fromisoformat(data["next_billing_date"]) if data.get("next_billing_date") else None
        subscription.resume_at = datetime.fromisoformat(data["resume_at"]) if data.get("resume_at") else None
...
if __name__ == "__main__":
    ...
    print(f"Trial end: {subscription.trial_end.strftime('%Y-%m-%d') if subscription.trial_end else 'N/A'}")
    print(f"Current period: {subscription.current_period_start.strftime('%Y-%m-%d')} to {subscription.current_period_end.strftime('%Y-%m-%d') if subscription.current_period_end else 'N/A'}")
    print(f"Next billing date: {subscription.next_billing_date.strftime('%Y-%m-%d') if subscription.next_billing_date else 'N/A'}")
    ...
    print(f"\nAfter cancellation:")
    print(f"Status: {subscription.status}")
    print(f"Canceled at: {subscription.canceled_at.strftime('%Y-%m-%d %H:%M:%S') if subscription.canceled_at else 'N/A'}")
