...
from .billing_calculator import PricingTier, PricingRule, PricingModel, BillingCalculator
...
class TieredPricingRule(PricingRule):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'TieredPricingRule':
        ...
        if "volume_discounts" in data:
            volume_discounts = [VolumeDiscount.from_dict(discount_data) for discount_data in data["volume_discounts"]]
...
class TieredPricingCalculator(BillingCalculator):
    ...
    def calculate_tiered_cost_breakdown(
        ...
        for tier in rule.tiers:
            tier_quantity = tier.get_quantity_in_tier(quantity) if graduated else (
                quantity if tier.contains(quantity) else 0.0
            )
        ...
        if hasattr(rule, "volume_discounts") and rule.volume_discounts:
            for discount in sorted(rule.volume_discounts, key=lambda d: d.min_quantity, reverse=True):
                if discount.applies_to(quantity):
