...
def demonstrate_formula_based_pricing() -> None:
    ...
    seasonal_discount = (
        ...
        if complex_rule.variables["time_month"] in [6, 7, 8]
        else 0
    )
    loyalty_discount = (
        complex_rule.variables["loyalty_discount"]
        * min(1, complex_rule.variables["customer_age"] / 365)
        * 100
    )
...
def demonstrate_combined_pricing() -> None:
    ...
    calculator.add_custom_rule(
        SeasonalPricingRule(
            metric=UsageMetric.STORAGE,
            seasonal_rates={"winter": 0.05, "summer": 0.03},
            default_rate=0.04,
            category=UsageCategory.STORAGE,
        )
    )
    ...
    calculator.add_custom_rule(
        CustomerSegmentPricingRule(
            metric=UsageMetric.TOKEN,
            segment_rates={"tier:premium": 0.0015, "tier:enterprise": 0.001},
            default_rate=0.002,
            category=UsageCategory.INFERENCE,
        )
    )
