...
def demo_content_comparison():
    ...
    if metric_data:
        ...
        if metric_data["highest_value"]["content_id"]:
            ...
            print(f"  Best performing: {best_title} with {metric_data['highest_value']['value']} views")
...
def demo_identify_top_content():
    ...
    for i, content in enumerate(top_content):
        ...
        print(f"   Key metrics: {', '.join([f'{k}: {v}' for k, v in list(content['metrics'].items())[:3]])}")
        print()
...
def demo_content_reports():
    ...
    print(f"Time Period: {social_report['time_period']['start']} to {social_report['time_period']['end']}")
