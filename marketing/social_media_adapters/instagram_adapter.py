...
class InstagramAdapter(BaseSocialMediaAdapter):
    ...
    def __init__(self, connection_id: str, connection_data: Dict[str, Any]):
        ...
        self.facebook_page_id = self.credentials.get("facebook_page_id")  # Required for some operations
    ...
    def validate_content(self, content: Dict[str, Any]) -> bool:
        ...
        if "image" in content:
            if "url" not in content["image"] and "source" not in content["image"]:
                raise ContentValidationError(
                    "instagram",
                    "Image must have either a URL or source (base64 data)"
                )
        ...
        if "video" in content:
            if "url" not in content["video"] and "source" not in content["video"]:
                raise ContentValidationError(
                    "instagram",
                    "Video must have either a URL or source (file path)"
                )
        ...
        if "carousel" in content:
            if not isinstance(content["carousel"], list) or len(content["carousel"]) < 2:
                raise ContentValidationError(
                    "instagram",
                    "Carousel must be a list with at least 2 items"
                )
            ...
            for item in content["carousel"]:
                ...
                if item["type"] == "image" and "url" not in item and "source" not in item:
                    raise ContentValidationError(
                        "instagram",
                        "Carousel image must have either a URL or source"
                    )
                
                if item["type"] == "video" and "url" not in item and "source" not in item:
                    raise ContentValidationError(
                        "instagram",
                        "Carousel video must have either a URL or source"
                    )
        ...
        if "story" in content:
            if "type" not in content["story"]:
                raise ContentValidationError(
                    "instagram",
                    "Story must have a 'type' field (image or video)"
                )
            ...
            if content["story"]["type"] == "image" and "url" not in content["story"] and "source" not in content["story"]:
                raise ContentValidationError(
                    "instagram",
                    "Story image must have either a URL or source"
                )
            
            if content["story"]["type"] == "video" and "url" not in content["story"] and "source" not in content["story"]:
                raise ContentValidationError(
                    "instagram",
                    "Story video must have either a URL or source"
                )
    ...
    def _post_carousel(self, carousel: List[Dict[str, Any]], post_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def schedule_post(
        ...
        try:
            # Check if we have an Instagram account ID (required for posting)
            if not self.instagram_account_id:
                raise SchedulingError("instagram", "Instagram account ID is required for scheduling posts")
            ...
            return {
                ...
                "status": "scheduled",
                "platform_data": {
                    "content_type": "image" if "image" in content else "video" if "video" in content else "carousel",
