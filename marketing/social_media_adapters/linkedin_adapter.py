    ...
    def validate_content(self, content: Dict[str, Any]) -> bool:
        ...
        if "document" in content:
            if "url" not in content["document"] and "source" not in content["document"]:
                raise ContentValidationError(
                    "linkedin",
                    "Document must have either a URL or source (file path)"
                )
    ...
    def schedule_post(
        ...
        try:
            ...
            return {
                ...
                "status": "scheduled",
                "platform_data": {
                    "content_type": "text" if "text" in content else "article" if "article" in content else "image" if "image" in content else "video" if "video" in content else "document"
                }
            }
    ...
    def get_post(self, post_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.get(
                f"{self.api_base_url}/socialActions/{post_urn}"
            )
            response.raise_for_status()
    ...
    def delete_post(self, post_id: str) -> bool:
        ...
        try:
            ...
            response = self.session.delete(
                f"{self.api_base_url}/ugcPosts/{post_urn}"
            )
            response.raise_for_status()
    ...
    def get_analytics(
        ...
        try:
            ...
            # Otherwise, get organization-level analytics
                ...
                return {
                    "error": "No post ID or organization ID provided for analytics"
                }
    ...
    def get_audience_insights(
        ...
        try:
            ...
            if not self.organization_id:
                return {
                    "error": "Organization ID is required for audience insights"
                }
