...
class CampaignTracker(ICampaignTracker):
    ...
    def create_campaign(
        ...
        campaign = {
            ...
            "status": "active" if start_date <= now and (end_date is None or end_date >= now) else "scheduled",
    ...
    def update_campaign(
        ...
        **kwargs
        ...
        allowed_fields = [
            "name", "description", "channels", "goals", "target_metrics",
            "start_date", "end_date", "budget", "tags", "metadata", "status"
        ]
    ...
    def get_metrics(
        ...
        if aggregation not in valid_aggregations:
            raise InvalidParameterError(f"Invalid aggregation method: {aggregation}. Must be one of {valid_aggregations}")
        
        # Validate group_by parameter
        valid_groupings = [None, "channel", "daily", "weekly", "monthly", "metric"]
        if group_by not in valid_groupings:
            raise InvalidParameterError(f"Invalid group_by value: {group_by}. Must be one of {valid_groupings}")
                ...
                for channel_name, channel_metrics in channels.items():
                    ...
                    for metric_name, values in channel_metrics.items():
                        if aggregation == "sum":
                            ...
                            result["grouped_data"][channel_name][metric_name] = sum(values) / len(values)
                        elif aggregation == "min":
    ...
    def analyze_performance(
        ...
        result["overall_performance"] = sum(performance_scores) / len(performance_scores) if performance_scores else 0
        ...
        for group, group_data in result["groups_performance"].items():
            ...
            group_data["average_achievement"] = sum(achievements) / len(achievements) if achievements else 0
    ...
    def compare_campaigns(
        ...
        for metric_name in metric_names:
            ...
            metric_comparison["average_achievement"] = sum(achievements) / len(achievements) if achievements else 0
    ...
    def generate_report(
        ...
        report = {
            ...
            "time_period": {
                ...
                "end": end_date.isoformat() if end_date else (campaign.get("end_date") or datetime.now().isoformat())
            },
            "performance_summary": {}
        }
        ...
        sorted_metrics = sorted(
            performance["metrics_achievement"].items(),
            key=lambda x: x[1],
            reverse=True
        )
