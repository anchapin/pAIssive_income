...
class ContentPerformanceAnalyzer(IContentPerformanceAnalyzer):
                ...
                for channel, channel_data in channels_data.items():
                    ...
                    for engagement_type, values in channel_data.items():
                        if aggregation == "sum":
                            ...
                            result["grouped_data"][channel][engagement_type] = sum(values) / len(values)
                        elif aggregation == "min":
                ...
                for period, period_data in sorted(time_periods.items()):
                    ...
                    for engagement_type, values in period_data.items():
                        if aggregation == "sum":
                            ...
                            result["grouped_data"][period][engagement_type] = sum(values) / len(values)
                        elif aggregation == "min":
    ...
    def analyze_performance(
        ...
        if performance:
            pct_values = [p["performance_percentage"] for p in performance.values() if p["performance_percentage"] != float('inf')]
        ...
        engagements = sum(metrics["aggregate"].get(e, 0) for e in ["click", "like", "comment", "share"])
        
        engagement_rate = (engagements / views) * 100 if views > 0 else 0
        
        # Calculate conversion rate
        conversions = sum(metrics["aggregate"].get(c, 0) for c in ["lead", "signup", "download", "purchase"])
        ...
        if report_type == "summary":
            ...
            for metric_name, value in sorted_metrics[:5]:
                report["performance_summary"]["top_metrics"].append({
                    "name": metric_name,
                    "value": value,
                    "benchmark": analysis["performance"].get(metric_name, {}).get("benchmark", 0),
                    "performance": analysis["performance"].get(metric_name, {}).get("performance_percentage", 0)
                })
