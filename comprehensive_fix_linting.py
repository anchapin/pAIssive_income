...
(file_path):
    ...
    try:
        ...
        print(f"Syntax error in {file_path}, skipping import fixes")
        return content
        ...
        if isinstance(imp, ast.Import):
            ...
            for name in imp.names:
                if name.name == "*":
                    ...
                    break
                if name.asname:
                    if name.asname in used_names:
                        all_unused = False
                        break
                elif name.name in used_names:
                    all_unused = False
                    break
            if all_unused:
                to_remove.append(
...
def process_file(file_path):
    ...
    try:
        ...
        content = fix_unused_variables(file_path)
        content = fix_import_order(file_path)
