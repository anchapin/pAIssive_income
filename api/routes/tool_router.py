...
async def add_endpoint(payload: BinaryOpRequest, api_key: str = Depends(api_key_auth)):
...
async def subtract_endpoint(
    payload: BinaryOpRequest, api_key: str = Depends(api_key_auth)
):
...
async def divide_endpoint(
    ...
    try:
        ...
        logger.info(
            f"[AUDIT] tool=divide, params={payload.dict()}, api_key=***, error=ZeroDivisionError"
        )
...
async def average_endpoint(
    payload: AverageRequest, api_key: str = Depends(api_key_auth)
):
    try:
        result = average(payload.numbers)
    except ValueError:
        logger.info(
            f"[AUDIT] tool=average, params={payload.dict()}, api_key=***, error=ValueError"
        )
        raise HTTPException(
