...
class WebhookRepository:
    ...
    def create_webhook(self, url: str, events: List[WebhookEventType], 
        ...
        webhook = {
            ...
            "events": [event.value if isinstance(event, WebhookEventType) else event for event in events],
            "active": active,
            "created_at": now,
            "updated_at": None,
    ...
    def update_webhook(self, webhook_id: str, 
        ...
        if events is not None:
            webhook["events"] = [event.value if isinstance(event, WebhookEventType) else event for event in events]
    ...
    def get_webhooks_for_event(self, event: str) -> List[Dict[str, Any]]:
        ...
        return [self.webhooks[wid] for wid in webhook_ids if wid in self.webhooks and self.webhooks[wid]["active"]]
    
    def create_delivery(self, webhook_id: str, event_type: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        logger.info(f"Created delivery {delivery_id} for webhook {webhook_id} and event {event_type}")
        return delivery
    
    def create_delivery_attempt(self, delivery_id: str, request_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def update_delivery_attempt(self, attempt_id: str, status: WebhookDeliveryStatus,
    ...
    def get_deliveries(self, webhook_id: Optional[str] = None, 
                      limit: int = 100, 
                      offset: int = 0,
                      status: Optional[WebhookDeliveryStatus] = None) -> List[Dict[str, Any]]:
        """
    ...
    def count_deliveries(self, webhook_id: Optional[str] = None, 
                        status: Optional[WebhookDeliveryStatus] = None) -> int:
        """
