...
class APIServer:
    ...
    def _setup_version_routes(self, version: APIVersion) -> None:
        ...
        self.app.include_router(user_router, prefix=f"{version_prefix}/user", tags=["User"])
        ...
        if self.config.enable_monetization:
            self.app.include_router(
                monetization_router, prefix=f"{version_prefix}/monetization", tags=["Monetization"]
            )

        if self.config.enable_marketing:
            self.app.include_router(
                marketing_router, prefix=f"{version_prefix}/marketing", tags=["Marketing"]
            )

        if self.config.enable_ai_models:
            self.app.include_router(
                ai_models_router, prefix=f"{version_prefix}/ai-models", tags=["AI Models"]
            )

        if self.config.enable_agent_team:
            self.app.include_router(
                agent_team_router, prefix=f"{version_prefix}/agent-team", tags=["Agent Team"]
            )

        if self.config.enable_dashboard:
            self.app.include_router(
                dashboard_router, prefix=f"{version_prefix}/dashboard", tags=["Dashboard"]
            )

        if self.config.enable_analytics:
            self.app.include_router(
                analytics_router, prefix=f"{version_prefix}/analytics", tags=["Analytics"]
            )

        if self.config.enable_developer:
            self.app.include_router(
                developer_router, prefix=f"{version_prefix}/developer", tags=["Developer"]
            )
