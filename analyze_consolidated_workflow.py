...
def analyze_workflow():
    ...
    for i, line in enumerate(lines, 1):
        ...
        if 'run:' in line and ('"' in line or "'" in line):
            ...
            while j < len(lines) and j < i + 50:  # Check next 50 lines max
                if '\\' in lines[j-1] and ('\ ' in lines[j-1] or '\\\r' in lines[j-1]):
                    has_backslash = True
                    end_line = j
    ...
    print('=== PROBLEMATIC MULTILINE STRING PATTERNS ===')
    for line_start, line_end, line_content in problematic_patterns:
        print(f'Lines {line_start}-{line_end}: {line_content[:60]}...')
    ...
    for i, line in enumerate(lines, 1):
        if 'run:' in line and '"' in line:
            ...
            if line.count('"') % 2 != 0:
                print(f'Line {i}: Unmatched quotes - {line.strip()}')
