...
def fix_workflow_triggers() -> None:
    ...
    default_triggers = {
        "on": {
            ...
            "pull_request": {"branches": ["main", "develop", "master"]},
            "workflow_dispatch": {},
        }
    }
            ...
            try:
                ...
                if isinstance(workflow, dict) and "on" not in workflow:
                    ...
                    with file_path.open("w", encoding="utf-8") as f:
                        yaml.dump(
                            workflow, f, default_flow_style=False, sort_keys=False
                        )
    ...
    if package_json.exists():
        try:
            ...
            if "tailwind:build" not in data["scripts"]:
                data["scripts"]["tailwind:build"] = (
                    "tailwindcss -i ./ui/static/css/tailwind.css -o ./ui/static/css/tailwind.output.css --minify"
                )
