    ...
    def publish(
        ...
        try:
            ...
            properties = pika.BasicProperties(
                ...
                expiration=str(int((message.expires_at - time.time()) * 1000)) if message.expires_at else None,
...
class AsyncMessageQueueClient:
    ...
    async def bind_queue(
        ...
        try:
            ...
            logger.info(f"Bound queue {queue.name} to exchange {exchange_name} with routing key {routing_key}")
    ...
    async def request(
        ...
        consumer_tag = await queue.consume(
            ...
            consumer_tag=f"request-{correlation_id}"
        )
