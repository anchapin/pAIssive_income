...
def event_driven_workflow_example():
    ...
    niche_selected_schema = EventSchema(
        data_model=NicheSelected, event_name="workflow.niche.selected"
    )

    solution_created_schema = EventSchema(
        data_model=SolutionCreated, event_name="workflow.solution.created"
    )
    ...
    try:
        ...
        def handle_niche_selected(event: Event):
            ...
            logger.info(
                f"Solution Service: Received niche selected event for {niche_data.niche_name}"
            )
        ...
        solution_bus.subscribe(
            event_pattern="workflow.niche.selected", handler=handle_niche_selected
        )

        marketing_bus.subscribe(
            event_pattern="workflow.solution.created", handler=handle_solution_created
        )
