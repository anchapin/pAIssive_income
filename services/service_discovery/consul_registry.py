...
class ConsulServiceRegistry(ServiceRegistry):
    ...
    def register(self, service_instance: ServiceInstance) -> bool:
        ...
        try:
            ...
            if result:
                logger.info(f"Service {service_instance.service_name} registered successfully with ID {service_instance.service_id}")
                return True
            else:
    ...
    def renew(self, service_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"TTL renewal not necessary for service {service_id} as we use HTTP checks")
            return True
        except Exception as e:
        ...
        try:
            ...
            for check in checks:
                if check["Status"] != "passing":
                    logger.warning(f"Service {service_id} has non-passing check: {check['Name']} - {check['Status']}")
                    return False
