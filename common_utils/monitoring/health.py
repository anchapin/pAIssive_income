...
class HealthCheckRegistry:
    ...
    def __new__(cls):
        with cls._lock:
            if cls._instance is None:
                ...
                cls._instance._health_checks: Dict[str, Callable[[], Tuple[HealthStatus, Dict[str, Any]]]] = {}
                cls._instance._results_cache: Dict[str, Dict[str, Any]] = {}
                cls._instance._cache_timestamp: Dict[str, float] = {}
                cls._instance._cache_duration = 60  # Cache health check results for 60 seconds by default
        return cls._instance
    
    def register_check(
        self, 
        name: str, 
        check_func: Callable[[], Tuple[HealthStatus, Dict[str, Any]]]
    ) -> None:
        """
    ...
    def run_check(self, name: str) -> Tuple[HealthStatus, Dict[str, Any]]:
        ...
        if name not in self._health_checks:
            ...
            return HealthStatus.UNKNOWN, {"error": f"Health check '{name}' not registered"}
        
        # Check if we have a recent cached result
        current_time = time.time()
        if (name in self._cache_timestamp and 
            current_time - self._cache_timestamp[name] < self._cache_duration):
            return self._results_cache[name]["status"], self._results_cache[name]["details"]
...
def database_health_check(db_connection) -> Callable[[], Tuple[HealthStatus, Dict[str, Any]]]:
    """
...
def api_health_check(api_url: str, timeout: float = 5.0) -> Callable[[], Tuple[HealthStatus, Dict[str, Any]]]:
    """
