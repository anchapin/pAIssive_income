...
async def read_file_async(file_path: str, binary: bool = False, encoding: str = 'utf-8') -> Union[str, bytes]:
    """
...
async def write_file_async(file_path: str, content: Union[str, bytes], binary: bool = False, 
                         encoding: str = 'utf-8', create_dirs: bool = True) -> None:
    """
...
async def list_files_async(directory: str, pattern: Optional[str] = None, 
                   recursive: bool = False) -> List[str]:
    """
...
async def list_directories_async(directory: str, pattern: Optional[str] = None, 
                        recursive: bool = False) -> List[str]:
    """
...
async def batch_process_files_async(file_paths: List[str], 
    ...
    async def process_file(file_path):
        async with semaphore:
            try:
                ...
                return {"file_path": file_path, "result": result, "success": True, "error": None}
            except Exception as e:
                logger.error(f"Error processing file {file_path}: {e}")
                return {"file_path": file_path, "result": None, "success": False, "error": str(e)}
...
async def read_files_batch_async(file_paths: List[str], binary: bool = False, 
                          encoding: str = 'utf-8', concurrency: int = 5) -> List[dict]:
    """
...
async def write_files_batch_async(file_contents: List[tuple], binary: bool = False, 
...
async def find_files_with_content_async(directory: str, content_pattern: str, 
    ...
    search_results = await batch_process_files_async(files_to_search, search_file_for_content, concurrency)
    
    # Return only files that contain the content
    matching_files = [result["file_path"] for result in search_results 
