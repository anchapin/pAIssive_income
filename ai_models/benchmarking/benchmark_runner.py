...
class BenchmarkRunner:
    ...
    def _load_input_data(self) -> List[str]:
        ...
        if self.config.input_data:
            ...
            # Default input data
            return ["Hello, world!", "How are you today?", "What is the meaning of life?"]
    ...
    def _load_model(self) -> Tuple[Any, Any]:
        ...
        if self.config.model_type == "text-generation":
            if not TRANSFORMERS_AVAILABLE:
                raise ImportError("Transformers library is required for text generation models")
            ...
            if not TRANSFORMERS_AVAILABLE:
                raise ImportError("Transformers library is required for embedding models")
    ...
    def _measure_throughput(self) -> float:
        ...
        if len(batch_inputs) < self.config.batch_size:
            ...
            batch_inputs = (batch_inputs * (self.config.batch_size // len(batch_inputs) + 1))[:self.config.batch_size]
        ...
        for _ in range(self.config.num_runs):
            with torch.no_grad():
                if self.config.model_type == "text-generation":
                    outputs = model.generate(**encoded_inputs, max_length=self.config.max_tokens)
                    total_output_tokens += outputs.numel()
                else:
                    model(**encoded_inputs)
                    total_output_tokens += input_token_count  # For non-generative models
    ...
    def _measure_perplexity(self) -> float:
        ...
        if not TRANSFORMERS_AVAILABLE:
            raise ImportError("Transformers library is required for perplexity calculation")
        ...
        try:
            if self.config.benchmark_type == BenchmarkType.LATENCY:
                ...
                if not reference_file:
                    raise ValueError("Reference file is required for accuracy benchmark")
                ...
                custom_benchmark_func = self.config.additional_params.get("benchmark_func")
                if not custom_benchmark_func:
                    raise ValueError("Custom benchmark function is required for custom benchmark")
