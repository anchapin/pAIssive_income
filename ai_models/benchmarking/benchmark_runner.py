...
class BenchmarkRunner:
    ...
    def run(self) -> BenchmarkResult:
        ...
        Returns:
            Benchmark result
        """
        logger.info(f"Running {self.config.benchmark_type.value} benchmark for {self.config.model_path}")
    ...
    def _run_throughput_benchmark(self) -> None:
        ...
        for i in range(0, len(self.input_data), self.config.batch_size):
            ...
            logger.info(f"Batch {i//self.config.batch_size + 1}: {len(batch)} samples, {batch_time:.2f} seconds")
    ...
    def _run_accuracy_benchmark(self) -> None:
        ...
        if not has_labels:
            raise ValueError("Input data must contain text and label fields for accuracy benchmarks")
    ...
    def _run_perplexity_benchmark(self) -> None:
        ...
        perplexity = torch.exp(total_loss / total_tokens).item() if total_tokens > 0 else float('inf')
    ...
    def _run_rouge_benchmark(self) -> None:
        ...
        if not has_references:
            raise ValueError("Input data must contain text and reference fields for ROUGE benchmarks")
