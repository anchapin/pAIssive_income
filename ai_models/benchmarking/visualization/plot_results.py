...
def plot_benchmark_results(
    ...
    **kwargs
    ...
    if result.benchmark_type == BenchmarkType.LATENCY:
        ...
        plt.text(0.5, 0.5, f"No plot available for {result.benchmark_type.value}",
                 ha='center', va='center', fontsize=14)
    
    # Add title
    plt.suptitle(f"{result.benchmark_type.value.capitalize()} Benchmark: {os.path.basename(result.model_path)}",
                fontsize=16)
...
def plot_latency_distribution(
    ...
    **kwargs
    ...
    for (label, value), color in zip(percentiles, colors):
        ...
        ax.text(value, ax.get_ylim()[1] * 0.9, f"{label}: {value:.2f} ms",
                color=color, ha='center', va='top', rotation=90)
...
def _plot_latency_results(fig: plt.Figure, result: BenchmarkResult, **kwargs) -> None:
    ...
    if not result.latency_ms:
        plt.text(0.5, 0.5, "No latency data available",
                 ha='center', va='center', fontsize=14)
        return
    ...
    for (label, value), color in zip(percentiles, colors):
        ...
        ax3.text(value, ax3.get_ylim()[1] * 0.9, f"{label}: {value:.2f} ms",
                 color=color, ha='center', va='top', rotation=90)
...
def _plot_throughput_results(fig: plt.Figure, result: BenchmarkResult, **kwargs) -> None:
    ...
    if result.throughput is None:
        plt.text(0.5, 0.5, "No throughput data available",
                 ha='center', va='center', fontsize=14)
        return
    ...
    ax.text(0, result.throughput, f"{result.throughput:.2f} tokens/s",
            ha='center', va='bottom')
    
    # Add batch size information
    if result.config and hasattr(result.config, "batch_size"):
        ax.text(0, result.throughput * 0.5, f"Batch Size: {result.config.batch_size}",
                ha='center', va='center')
...
def _plot_memory_results(fig: plt.Figure, result: BenchmarkResult, **kwargs) -> None:
    ...
    if not result.memory_usage_mb:
        plt.text(0.5, 0.5, "No memory data available",
                 ha='center', va='center', fontsize=14)
        return
    ...
    if "total_mb" in memory_usage:
        ax.text(0.5, max(values) * 1.1, f"Total: {memory_usage['total_mb']:.2f} MB",
...
def _plot_accuracy_results(fig: plt.Figure, result: BenchmarkResult, **kwargs) -> None:
    ...
    # Add value on top of bar
    ax.text(0, result.accuracy, f"{result.accuracy:.2f}%",
            ha='center', va='bottom')
...
def _plot_perplexity_results(fig: plt.Figure, result: BenchmarkResult, **kwargs) -> None:
    ...
    if result.perplexity is None:
        plt.text(0.5, 0.5, "No perplexity data available",
                 ha='center', va='center', fontsize=14)
        return
    ...
    # Add value on top of bar
    ax.text(0, result.perplexity, f"{result.perplexity:.2f}",
            ha='center', va='bottom')
    
    # Add note about perplexity
    ax.text(0, result.perplexity * 0.5, "Lower is better",
