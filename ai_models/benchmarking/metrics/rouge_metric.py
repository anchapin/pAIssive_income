...
class RougeMetric(BaseMetric):
    ...
    def measure(
        ...
        **kwargs
        ...
        avg_scores = {}
        for rouge_type in self.rouge_types:
            avg_scores[rouge_type] = sum(batch_scores[rouge_type]) / len(batch_scores[rouge_type]) if batch_scores[rouge_type] else 0
    ...
    def get_overall_scores(self) -> Dict[str, float]:
        ...
        overall_scores = {}
        for rouge_type in self.rouge_types:
            overall_scores[rouge_type] = sum(self.scores[rouge_type]) / len(self.scores[rouge_type]) if self.scores[rouge_type] else 0
        ...
        for rouge_type in self.rouge_types:
            if self.scores[rouge_type]:
                ...
                stats[rouge_type] = {
                    ...
                    "std_dev": statistics.stdev(self.scores[rouge_type]) if len(self.scores[rouge_type]) > 1 else 0
                }
