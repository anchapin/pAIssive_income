...
def setup_test_environment():
    ...
    models = [
        ModelInfo(id="model1", name="Primary Model", type="huggingface", path="/path/to/model1", 
                 capabilities=["text-generation", "classification"]),
        ModelInfo(id="model2", name="Secondary Model", type="llama", path="/path/to/model2",
                 capabilities=["text-generation"]),
        ModelInfo(id="model3", name="Embedding Model", type="embedding", path="/path/to/model3",
                 capabilities=["embedding"]),
        ModelInfo(id="model4", name="Small Model", type="huggingface", path="/path/to/model4",
...
def test_basic_fallback(manager: ModelManager):
    ...
    try:
        ...
        print(f"Successfully fell back to model: {model_info.name} ({model_info.id}) - Type: {model_info.type}")
