    ...
    def __init__(
        ...
        **kwargs
        ...
        if not TENSORRT_AVAILABLE:
            raise ImportError("TensorRT not available. Please install it to use the TensorRT adapter.")
        
        if not PYCUDA_AVAILABLE:
            raise ImportError("PyCUDA not available. Please install it to use the TensorRT adapter.")
        ...
        if self.model_type in ["text-generation", "text-classification", "embedding"] and self.tokenizer_path:
        ...
        try:
            ...
            for name in self.input_names:
                if name == "input_ids" and "input_ids" in inputs:
                    ...
                    model_inputs[name] = np.array([len(inputs["input_ids"][0]) + max_tokens], dtype=np.int32)
        ...
        try:
            ...
            if kwargs.get("normalize", True):
                if TORCH_AVAILABLE:
                    ...
                    embeddings = torch.nn.functional.normalize(embeddings_tensor, p=2, dim=1).numpy()
                else:
