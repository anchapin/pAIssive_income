...
class EnhancedInferenceMetrics(InferenceMetrics):
    ...
    def update_token_usage(
        ...
        prompt_time = self.time_to_first_token if self.time_to_first_token > 0 else self.total_time * 0.1
        completion_time = self.total_time - prompt_time if self.total_time > prompt_time else self.total_time * 0.9
...
class EnhancedPerformanceMonitor(PerformanceMonitor):
    ...
    def set_model_cost_rates(self, model_id: str, prompt_cost_per_1k: float, completion_cost_per_1k: float) -> None:
        """
        ...
        logger.info(f"Set cost rates for model {model_id}: prompt=${prompt_cost_per_1k}/1k, completion=${completion_cost_per_1k}/1k")
    ...
    def get_model_cost_rates(self, model_id: str) -> Dict[str, float]:
        ...
        """
        return self.model_cost_rates.get(model_id, {
            "prompt_cost_per_1k": 0.0,
            "completion_cost_per_1k": 0.0
        })
        
    def track_enhanced_inference(self, 
                               model_id: str, 
                               batch_id: str = None) -> 'EnhancedInferenceTracker':
        """
    ...
    def generate_enhanced_report(self, 
                               ...
                               batch_id: str = None,
                               include_metrics: bool = False) -> EnhancedPerformanceReport:
        """
    ...
    def compare_models_enhanced(self,
                              model_ids: List[str],
                              model_names: List[str] = None,
                              title: str = "Enhanced Model Comparison",
                              time_range: Tuple[datetime, datetime] = None) -> ModelComparisonReport:
        """
        ...
        metrics_to_compare = [
            "avg_inference_time", "median_inference_time",
            "min_inference_time", "max_inference_time",
            "avg_latency_ms", "avg_time_to_first_token",
            "avg_tokens_per_second", "avg_memory_usage_mb",
            "avg_cpu_percent", "avg_gpu_percent",
            "avg_prompt_tokens", "avg_completion_tokens", 
            "avg_completion_to_prompt_ratio",
            "cost_per_1k_tokens", "avg_queue_time_ms",
        ...
        comparison.calculate_percent_differences([
            "avg_inference_time", "avg_latency_ms",
            "avg_tokens_per_second", "cost_per_1k_tokens"
        ])
    ...
    def visualize_token_usage(self,
        ...
        if "total_tokens" in df.columns and df["total_tokens"].max() > 0:
            try:
                ...
                plt.bar(daily_df["date"], daily_df["prompt_tokens"], label="Prompt Tokens", alpha=0.7)
                plt.bar(daily_df["date"], daily_df["completion_tokens"], 
                      bottom=daily_df["prompt_tokens"], label="Completion Tokens", alpha=0.7)
        ...
        if "total_cost" in df.columns and df["total_cost"].max() > 0:
            try:
                ...
                plt.bar(daily_df["date"], daily_df["prompt_cost"], label="Prompt Cost", alpha=0.7)
                plt.bar(daily_df["date"], daily_df["completion_cost"], 
                      bottom=daily_df["prompt_cost"], label="Completion Cost", alpha=0.7)
                ...
                plt.text(0.02, 0.95, f"Total Cost: ${total_cost:.2f}", 
...
class EnhancedInferenceTracker:
    ...
    def _capture_system_metrics(self) -> None:
        ...
        try:
            ...
            try:
                import resource
                self.metrics.memory_usage_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024  # kB to MB
            except ImportError:
                pass
        except Exception as e:
