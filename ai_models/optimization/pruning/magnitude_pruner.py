...
class MagnitudePruner(Pruner):
    ...
    def _apply_one_shot_pruning(self, model, layers_to_prune: List[Tuple[torch.nn.Module, str]]) -> None:
        """
    ...
    def _apply_gradual_pruning(self, model, layers_to_prune: List[Tuple[torch.nn.Module, str]]) -> None:
        """
        ...
        Args:
            ...
            layers_to_prune: List of (module, name) tuples to prune
        """
        logger.info(f"Applying gradual pruning from {self.config.initial_sparsity} to {self.config.final_sparsity}")
    ...
    def get_pruning_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "initial_sparsity": self.config.initial_sparsity if self.config.gradual_pruning else self.config.sparsity,
            "final_sparsity": self.config.final_sparsity if self.config.gradual_pruning else self.config.sparsity
        }
