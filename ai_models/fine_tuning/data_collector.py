...
class DataCollector:
    ...
    def prepare(self) -> DatasetDict:
        ...
        if self.config.test_split > 0:
            test_valid_ratio = self.config.test_split / (self.config.validation_split + self.config.test_split)
        ...
        logger.info(f"Dataset split: {len(train_dataset)} train, {len(valid_dataset)} validation, {len(test_dataset)} test")
