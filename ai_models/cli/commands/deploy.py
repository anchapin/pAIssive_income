...
class DeployCommand(BaseCommand):
    ...
    def add_arguments(cls, parser: argparse.ArgumentParser) -> None:
        ...
        parser.add_argument(
            ...
            type=str,
            default="1",
            help="CPU limit for the deployment"
        )
        parser.add_argument(
            "--memory-limit",
            type=str,
    ...
    def _generate_cloud_config(self, config_dict: Dict[str, Any]) -> int:
        ...
        """
        # Import required modules
        from ...serving.deployment import CloudConfig, CloudProvider, generate_cloud_config
        ...
        config = CloudConfig(
            ...
            model_type=self.args.model_type,
            model_id=name,
            instance_type=self.args.instance_type or "ml.m5.large" if provider == CloudProvider.AWS else "n1-standard-2",
            cpu_count=int(self.args.cpu_limit.replace("m", "")) // 1000 if "m" in self.args.cpu_limit else int(self.args.cpu_limit),
            memory_gb=int(self.args.memory_limit.replace("Gi", "")) if "Gi" in self.args.memory_limit else int(self.args.memory_limit),
            gpu_count=self.args.gpu_count,
            min_instances=1,
