...
def test_quantization(
    ...
    try:
        ...
        print("\nSample Outputs:")
        for i, (prompt, orig, quant) in enumerate(zip(
            analysis["prompts"],
...
def test_pruning(
    ...
    try:
        ...
        # Print sample outputs
        print("\nSample Outputs:")
        for i, (prompt, orig, pruned) in enumerate(zip(
            analysis["prompts"],
            analysis["original_model"]["outputs"],
            analysis["pruned_model"]["outputs"]
        )):
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="optimized_models", help="Directory to save optimized models")
    parser.add_argument("--optimization", type=str, choices=["quantization", "pruning", "all"], default="all", help="Optimization to test")
    parser.add_argument("--quant-method", type=str, choices=["bitsandbytes-4bit", "bitsandbytes-8bit", "awq", "gptq"], default="bitsandbytes-4bit", help="Quantization method")
    parser.add_argument("--quant-bits", type=int, default=4, help="Number of bits for quantization")
    parser.add_argument("--prune-method", type=str, choices=["magnitude", "structured"], default="magnitude", help="Pruning method")
    parser.add_argument("--prune-sparsity", type=float, default=0.5, help="Target sparsity for pruning (0.0 to 1.0)")
