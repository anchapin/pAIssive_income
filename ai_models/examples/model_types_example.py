        ...
        if os.path.exists(audio_path):
            ...
            try:
                ...
                for event in events[:5]:  # Show only first 5 events
                    print(f"  {event['label']} ({event['score']:.4f}) at {event['start_time']:.2f}-{event['end_time']:.2f}s")
            except Exception as e:
...
def main():
    ...
    parser.add_argument("--model-type", type=str, choices=["onnx", "quantized", "vision", "audio"], help="Type of model to test")
    parser.add_argument("--model-path", type=str, help="Path to the model")
    parser.add_argument("--input-text", type=str, default="Hello, world!", help="Input text for text generation")
    ...
    parser.add_argument("--output-path", type=str, default="output.wav", help="Path to save synthesized speech")
