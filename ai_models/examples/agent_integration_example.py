...
from ai_models import ModelManager, ModelInfo, ModelConfig, AgentModelProvider
    ...
    if all_models:
        ...
        for agent_type, tasks in assignments.items():
            if agent_type == "developer" and "custom-task" in tasks:
                ...
                model_info = manager.get_model_info(model_id)
                print(f"Verified assignment: Agent: developer, Task: custom-task, Model: {model_info.name if model_info else model_id}")
