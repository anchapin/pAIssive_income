...
class ModelBenchmark:
    ...
    def _generate_comparison(self, model_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for model_id, data in model_results.items():
            ...
            # Check throughput
            if results["avg_tokens_per_second"] > comparison["highest_throughput"]["tokens_per_second"]:
                comparison["highest_throughput"]["model"] = model_name
                comparison["highest_throughput"]["tokens_per_second"] = results["avg_tokens_per_second"]
            
            # Check latency
            if results["avg_time_to_first_token"] < comparison["lowest_latency"]["time_to_first_token"]:
                comparison["lowest_latency"]["model"] = model_name
                comparison["lowest_latency"]["time_to_first_token"] = results["avg_time_to_first_token"]
        ...
        for model_id, data in model_results.items():
            ...
            if comparison["fastest_inference"]["time"] > 0:
                score += 0.3 * (results["avg_inference_time"] / comparison["fastest_inference"]["time"])
            
            if comparison["highest_throughput"]["tokens_per_second"] > 0:
                score += 0.3 * (1 - (results["avg_tokens_per_second"] / comparison["highest_throughput"]["tokens_per_second"]))
            
            if comparison["lowest_latency"]["time_to_first_token"] > 0:
                score += 0.2 * (results["avg_time_to_first_token"] / comparison["lowest_latency"]["time_to_first_token"])
...
def main():
    ...
    parser.add_argument("--prompt", type=str, default="The quick brown fox jumps over the lazy dog", help="Prompt to use for the benchmark")
    ...
    for model_id, data in results["models"].items():
        ...
        print(f"  Average time to first token: {results_data['avg_time_to_first_token']:.4f} seconds")
    ...
    if comparison["fastest_inference"]["model"]:
        print(f"Fastest inference: {comparison['fastest_inference']['model']} ({comparison['fastest_inference']['time']:.4f} seconds)")
    
    if comparison["highest_throughput"]["model"]:
        print(f"Highest throughput: {comparison['highest_throughput']['model']} ({comparison['highest_throughput']['tokens_per_second']:.2f} tokens/second)")
    
    if comparison["lowest_latency"]["model"]:
        print(f"Lowest latency: {comparison['lowest_latency']['model']} ({comparison['lowest_latency']['time_to_first_token']:.4f} seconds)")
    
    if comparison["lowest_memory"]["model"]:
        print(f"Lowest memory usage: {comparison['lowest_memory']['model']} ({comparison['lowest_memory']['memory_mb']:.2f} MB)")
