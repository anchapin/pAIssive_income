...
function buildTailwind(options = {}) {
  ...
  try {
    ...
    // Sanitize command construction to prevent injection
    const sanitizedFlags = [];
    if (minify) sanitizedFlags.push('--minify');
    if (watch) sanitizedFlags.push('--watch');
    const command = `${baseCommand} ${sanitizedFlags.join(' ')}`;
