...
class TestPaymentGatewayIntegration:
    ...
    processed_transaction = self.transaction_manager.process_transaction(
    transaction.id
    )
    ...
    retried_transaction = failing_transaction_manager.get_transaction(
    transaction.id
    )
    self.assertEqual(retried_transaction.status, TransactionStatus.FAILED)
    self.assertEqual(
    retried_transaction.retry_count, 4
    ...
    successful_transaction = retry_transaction_manager.get_transaction(
