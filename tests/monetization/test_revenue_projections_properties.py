...
def test_invariant_properties(
    name, description, initial_users, user_acquisition_rate, conversion_rate, churn_rate, tier_dist
):
...
@given(
    acquisition_cost=acquisition_costs, average_revenue=average_revenues, gross_margin=gross_margins
)
...
def test_revenue_projection_calculation_properties(
    ...
    custom_prices = {basic_tier["id"]: 14.99, pro_tier["id"]: 29.99, premium_tier["id"]: 59.99}

    revenue_projections_custom = projector.project_revenue(
        months=months, growth_rate=growth_rate, subscription_model=model, prices=custom_prices
    )
...
def test_revenue_projection_monthly_to_annual_conversion(
    initial_users, user_acquisition_rate, conversion_rate, churn_rate, months, growth_rate
):
