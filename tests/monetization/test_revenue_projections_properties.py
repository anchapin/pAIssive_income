...
def tier_distributions(draw):
    ...
    for i in range(num_tiers - 1):
        if remaining <= \
            0.01:  # Ensure we don't try to draw a value smaller than min_value
...
def test_revenue_projections_properties(
    ...
    # Project revenue
    revenue_projections = projector.project_revenue(months=months, 
    ...
    for projection in revenue_projections:
        assert projection["total_users"] == projection["free_users"] + \
            projection["paid_users"]
    ...
    for projection in revenue_projections:
        ...
        if projection["paid_users"] == 0:
            ...
            max_diff = min(
                len(tier_dist) + 1, projection["paid_users"]
            )  # Allow for rounding in each tier
...
def test_invariant_properties(
    name, description, initial_users, user_acquisition_rate, conversion_rate, churn_rate, 
        tier_dist
):
...
def test_lifetime_value_calculation_properties(average_revenue, churn_rate):
    ...
    assert abs(ltv["lifetime_value"] - \
        (average_revenue * ltv["average_lifetime_months"])) < 0.001
...
def test_payback_period_calculation_properties(acquisition_cost, average_revenue, 
    gross_margin):
...
def test_revenue_projection_calculation_properties(
    ...
    revenue_projections_default = projector.project_revenue(months=months, 
        growth_rate=growth_rate)
    ...
    custom_prices = {basic_tier["id"]: 14.99, pro_tier["id"]: 29.99, 
    ...
    if months > 0:
        ...
        for month in range(min(months, 
            12)):  # Check first 12 months or all months if fewer
            ...
            if matching_tiers and all(
                proj_model["tier_users"][t] == \
                    proj_custom["tier_users"][t] for t in matching_tiers
            ):
...
def test_revenue_projection_monthly_to_annual_conversion(
    initial_users, user_acquisition_rate, conversion_rate, churn_rate, months, 
        growth_rate
):
    ...
    if (
        ...
        assert increasing_pairs >= len(annual_revenues) - \
            2  # Allow one potential decrease
