...
class TestRevenueAnalytics:
    ...
    with patch.object(
    self.revenue_analytics, "get_active_subscriptions"
    ) as mock_subs:
    ...
    with patch.object(
    self.revenue_analytics, "get_subscription_events"
    ) as mock_events:
    ...
    with patch.object(
    self.churn_analyzer, "calculate_churn_rate"
    ) as mock_churn:
    ...
    assert (
    avg_clv_by_plan["plan_enterprise"] > avg_clv_by_plan["plan_premium"]
    )
    ...
    with patch.object(
    self.churn_analyzer, "predict_churn_risk"
    ) as mock_predict:
    ...
    at_risk_customers = self.churn_analyzer.get_at_risk_customers(
    risk_threshold=0.5
    )
    ...
    lambda customer_id: prevention_strategies.get(customer_id, [])
    )

    # Get prevention strategies for at-risk customer
    strategies = self.churn_analyzer.get_prevention_strategies(
    "cust_002"
    )
    ...
    with patch.object(
    self.revenue_analytics, "calculate_growth_rate"
    ) as mock_growth:
    ...
    with patch.object(
    self.revenue_analytics, "get_plan_distribution"
    ) as mock_dist:
    ...
    # Check first month
    assert plan_projection[0][
    "plan_basic"
    ] == current_mrr * Decimal("0.1")
    assert plan_projection[0][
    "plan_premium"
    ] == current_mrr * Decimal("0.3")
    assert plan_projection[0][
    "plan_enterprise"
    ] == current_mrr * Decimal("0.6")
