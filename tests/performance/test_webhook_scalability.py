        ...
        try:
            ...
            with patch("httpx.AsyncClient.post", return_value=mock_response):
                ...
                self.results["events_per_second"] = (
                    total_deliveries / self.results["total_duration"]
                ...
                successful_deliveries = sum(
                    ...
                    and d["status"] == WebhookDeliveryStatus.SUCCESS
                )
                ...
                if self.results["cpu_usage"]:
                    self.results["avg_cpu_percent"] = sum(
                        self.results["cpu_usage"]
                    ) / len(self.results["cpu_usage"])
                    self.results["max_cpu_percent"] = max(self.results["cpu_usage"])

                if self.results["memory_usage"]:
                    self.results["avg_memory_mb"] = sum(
                        self.results["memory_usage"]
                    ) / len(self.results["memory_usage"])
...
async def run_scalability_tests():
    ...
    with open("webhook_scalability_results.json", "w") as f:
        ...
        for r in results:
            ...
            if "start_time" in r_copy:
                r_copy["start_time"] = datetime.fromtimestamp(
                    r_copy["start_time"]
                ).isoformat()
            if "end_time" in r_copy:
                r_copy["end_time"] = datetime.fromtimestamp(
                    r_copy["end_time"]
                ).isoformat()
    ...
    for r in results:
        ...
        print(
            f"{r['scaling_factor']:14} | {r['events_per_second']:13.2f} | {r['success_rate']*100:10.2f}% | {cpu_percent:9.2f} | {memory_mb:14.2f}"
        )
    ...
    if base_result:
        ...
        for r in results:
            if r["scaling_factor"] > 1:
                ...
                efficiency = (
                    actual_throughput / ideal_throughput if ideal_throughput > 0 else 0
                )

                print(
                    f"{r['scaling_factor']:14} | {ideal_throughput:15.2f} | {actual_throughput:16.2f} | {efficiency*100:8.2f}%"
                )
