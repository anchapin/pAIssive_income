...
def generate_ai_model_data() -> Dict[str, Any]:
    ...
    return {
        ...
        "model_type": random.choice(["text-generation", "text-classification", "embedding", "image", "audio"]),
        "provider": random.choice(["openai", "ollama", "lmstudio", "huggingface"]),
        "version": f"{random.randint(1, 5)}.{random.randint(0, 9)}.{random.randint(0, 9)}",
        "capabilities": [
            random.choice(["text-generation", "text-classification", "embedding", "image-generation", "audio-transcription"])
        ...
        "agents": [
            {
                ...
                "capabilities": [
                    random.choice(["market_analysis", "problem_identification", "solution_development", "monetization_strategy", "marketing_plan"])
                    for _ in range(random.randint(1, 3))
                ]
            }
...
def generate_webhook_data() -> Dict[str, Any]:
    ...
    return {
        ...
        "event_types": [
            random.choice(["niche.created", "solution.created", "monetization.created", "marketing.created"])
