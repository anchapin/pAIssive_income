...
def test_webhook_delivery_metrics(reset_metrics):
    ...
    assert WEBHOOK_DELIVERIES_TOTAL.labels(
        webhook_id="test-webhook-1",
        event_type="user.created",
        status="success"
    )._value.get() == 1
    ...
    assert WEBHOOK_DELIVERY_DURATION.labels(
        webhook_id="test-webhook-1",
        event_type="user.created"
    )._sum.get() == 0.8  # 0.5 + 0.3
...
def test_health_metrics(mock_webhook_health, reset_metrics):
    ...
    mock_webhook_health.labels.assert_any_call(webhook_id="test-webhook-1", url="https://example.com/webhook1")
    mock_webhook_health.labels.assert_any_call(webhook_id="test-webhook-2", url="https://example.com/webhook2")
