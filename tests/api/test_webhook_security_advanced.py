...
This module tests advanced security features for the webhook system:
    ...
    assert nonce_verifier.verify_signature_with_nonce(secret, payload_str, signature, used_nonces) is True
    assert nonce in used_nonces

    # Try to verify the same signature again (should fail due to used nonce)
    assert nonce_verifier.verify_signature_with_nonce(secret, payload_str, signature, used_nonces) is False
    ...
    assert verifier.verify_signature_with_timestamp(secret, payload, old_signature, max_age_minutes=15) is True
