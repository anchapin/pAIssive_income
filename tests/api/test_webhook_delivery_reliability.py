...
class TestSlowEndpoints:
    """Tests for handling of slow responding endpoints."""
...
class TestIntermittentFailures:
    """Tests for delivery to endpoints with intermittent failures."""
...
class TestDeliveryOrdering:
    """Tests for delivery ordering guarantees."""
        ...
        try:
            ...
            with patch("httpx.AsyncClient.post", mock_post):
                ...
                for i in range(len(delivered_events) - 1):
                    assert (
                        delivered_events[i]["data"]["sequence"]
                        < delivered_events[i + 1]["data"]["sequence"]
                    )
...
class TestVaryingPayloadSizes:
    """Tests for delivery with varying payload sizes."""
            ...
            with patch("httpx.AsyncClient.post", return_value=success_response):
                ...
                large_data = {
                    ...
                    "profile": {
                        ...
                        "interests": ["coding", "testing", "webhooks"]
...
class TestDifferentEventTypes:
    """Tests for delivery across different event types."""
