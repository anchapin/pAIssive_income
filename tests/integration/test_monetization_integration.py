...
def test_subscription_model_to_revenue_projector_integration():
    ...
    for month in revenue_projections:
        assert month["tier_revenue"]["Basic"] == month["tier_users"]["Basic"] * 9.99
        assert month["tier_revenue"]["Pro"] == month["tier_users"]["Pro"] * 19.99
        assert month["tier_revenue"]["Premium"] == month["tier_users"]["Premium"] * 29.99
        assert month["total_revenue"] == sum(month["tier_revenue"].values())
...
def test_end_to_end_monetization_workflow(temp_subscription_dir):
    ...
    model.update_tier_price(business_tier["id"], price_monthly=business_price, price_yearly=business_price * 10)
