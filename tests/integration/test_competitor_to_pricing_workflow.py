...
def test_competitor_to_pricing_workflow(
    ...
    for tier in subscription_model.tiers:
        ...
        optimal_price = pricing_calculator.calculate_optimal_price(
            tier_name=tier["name"],
            cost_per_user=5.0 * (1 + subscription_model.tiers.index(tier) * 0.5),  # Higher tiers cost more
            value_perception=0.7 + (subscription_model.tiers.index(tier) * 0.1),  # Higher tiers have higher value perception
            competitor_price=competitor_price,
            price_sensitivity=0.8 - (subscription_model.tiers.index(tier) * 0.1)  # Higher tiers have lower price sensitivity
        )
    ...
    assert subscription_model.tiers[1]["price_monthly"] > subscription_model.tiers[0]["price_monthly"]
    assert subscription_model.tiers[2]["price_monthly"] > subscription_model.tiers[1]["price_monthly"]
    ...
    assert revenue_projection["revenue"][-1] > revenue_projection["revenue"][0]  # Revenue should increase over time
...
def test_competitor_to_pricing_workflow_with_freemium(
    ...
    for tier in freemium_model.tiers[1:]:  # Skip free tier
        ...
        optimal_price = pricing_calculator.calculate_optimal_price(
            ...
            value_perception=0.7 + ((freemium_model.tiers.index(tier) - 1) * 0.1),  # Higher tiers have higher value perception
            competitor_price=competitor_price,
            price_sensitivity=0.8 - ((freemium_model.tiers.index(tier) - 1) * 0.1)  # Higher tiers have lower price sensitivity
        )
    ...
    user_growth_rate = (last_month_users / first_month_users) if first_month_users > 0 else float('inf')
    revenue_growth_rate = (last_month_revenue / first_month_revenue) if first_month_revenue > 0 else float('inf')
