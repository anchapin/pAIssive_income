...
class TestAPIGatewayCircuitBreaker:
    ...
    def test_circuit_breaker_integration(self):
        ...
        service_responses = {
            ...
            "product-service": {"status": 200, "data": {"products": [{"id": 1, "name": "Test Product"}]}}
        }
        ...
        def mock_forward_request(request_data):
            ...
            return service_responses.get(service, {"status": 404, "data": {"error": "Service not found"}})
        
        # Patch the forward request method
        with patch("services.gateway.APIGateway._forward_request", side_effect=mock_forward_request):
            ...
            with patch("services.gateway.APIGateway._forward_request", side_effect=circuit_wrapped_forward):
    ...
    def test_partial_failures(self):
        ...
        with patch("services.gateway.APIGateway._forward_request", side_effect=circuit_wrapped_forward):
            ...
            for _ in range(5):
                requests.get("http://localhost:8000/api/products")
