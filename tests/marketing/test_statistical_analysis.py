...
class TestStatisticalAnalysis:
    ...
    def test_median_and_iqr(self):
        ...
        assert (
            iqr == 2.5
        )  # Q3 (4.5) - Q1 (2) = 2.5 using NumPy's default percentile method
    ...
    def test_chi_square_test(self):
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.chi_square_test(
                np.array([1, 2, 3])
            )  # 1D array without expected values
    ...
    def test_z_test_proportions(self):
        ...
        result = stats_analysis.z_test_proportions(
    ...
    def test_confidence_interval_mean(self):
        ...
        assert (
            result_90["margin_of_error"]
            < result["margin_of_error"]
            < result_99["margin_of_error"]
        )
    ...
    def test_confidence_interval_proportion(self):
        ...
        # Test with invalid input
        with pytest.raises(InvalidParameterError):
            stats_analysis.confidence_interval_proportion(
                count=40, nobs=100, method="invalid"
            )

        with pytest.raises(InvalidParameterError):
            stats_analysis.confidence_interval_proportion(
                count=40, nobs=100, confidence_level=1.5
            )
    ...
    def test_cohens_d(self):
        ...
        small_effect = stats_analysis.cohens_d(
    ...
    def test_odds_ratio(self):
        ...
        # Check specific values
        assert result["odds_ratio"] == pytest.approx(
            3.857, abs=1e-3
        )  # (30*90)/(70*10) = 3.857
    ...
    def test_relative_risk(self):
        ...
        # Check specific values
        assert result["risk_exposed"] == pytest.approx(
            30 / 100, abs=1e-10
        )  # 30/(30+70) = 0.3
        assert result["risk_unexposed"] == pytest.approx(
            10 / 100, abs=1e-10
        )  # 10/(10+90) = 0.1
    ...
    def test_number_needed_to_treat(self):
        ...
        # Check specific values
        assert result["risk_treatment"] == pytest.approx(
            20 / 100, abs=1e-10
        )  # 20/(20+80) = 0.2
        assert result["risk_control"] == pytest.approx(
            40 / 100, abs=1e-10
        )  # 40/(40+60) = 0.4
        ...
        # Test with invalid input
        with pytest.raises(InvalidParameterError):
            stats_analysis.number_needed_to_treat(
                [[1, 2, 3], [4, 5, 6]]
            )  # Not a 2x2 table
    ...
    def test_sample_size_for_proportion_test(self):
        ...
        result_large_effect = stats_analysis.sample_size_for_proportion_test(
            effect_size=0.2
        )
    ...
    def test_sample_size_for_mean_test(self):
        ...
        # Larger effect size should require smaller sample size
        result_large_effect = stats_analysis.sample_size_for_mean_test(
            effect_size=1.0, std_dev=1.0
        )
        assert result_large_effect["sample_size"] < result["sample_size"]
    ...
    def test_sample_size_for_correlation(self):
        ...
        # Larger effect size should require smaller sample size
        result_large_effect = stats_analysis.sample_size_for_correlation(
            effect_size=0.5
        )
        assert result_large_effect["sample_size"] < result["sample_size"]

        # Higher power should require larger sample size
        result_high_power = stats_analysis.sample_size_for_correlation(
            effect_size=0.3, power=0.9
        )
        assert result_high_power["sample_size"] > result["sample_size"]
    ...
    def test_type_error_rates(self):
        ...
        assert result_prop["power"] + result_prop["type_ii_error"] == pytest.approx(
            1.0, abs=1e-10
        )
        assert result_prop["effect_size"] == 0.1
        assert result_prop["sample_size"] == 400
        ...
        assert result_mean["power"] + result_mean["type_ii_error"] == pytest.approx(
            1.0, abs=1e-10
        )
        assert result_mean["test_type"] == "mean"
        ...
        assert result_corr["power"] + result_corr["type_ii_error"] == pytest.approx(
            1.0, abs=1e-10
        )
        assert result_corr["test_type"] == "correlation"
    ...
    def test_bonferroni_correction(self):
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.bonferroni_correction(
                [0.1, -0.1, 0.3]
            )  # Invalid p-value < 0
    ...
    def test_holm_bonferroni_correction(self):
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.holm_bonferroni_correction(
                [0.1, 1.1, 0.3]
            )  # Invalid p-value > 1
    ...
    def test_benjamini_hochberg_correction(self):
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.benjamini_hochberg_correction(
                [0.1, 1.1, 0.3]
            )  # Invalid p-value > 1
    ...
    def test_benjamini_yekutieli_correction(self):
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.benjamini_yekutieli_correction(
                [0.1, 1.1, 0.3]
            )  # Invalid p-value > 1
    ...
    def test_adjust_alpha(self):
        ...
        assert result["adjusted_alpha"] == pytest.approx(
            1.0 - (1.0 - 0.05) ** (1.0 / 5), abs=1e-10
        )
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.adjust_alpha(
                alpha=0.05, n_tests=5, method="invalid"
            )  # Invalid method
    ...
    def test_obrien_fleming_boundary(self):
        ...
        assert (
            result["z_boundaries"][0]
            > result["z_boundaries"][1]
            > result["z_boundaries"][2]
        )
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.obrien_fleming_boundary(
                num_looks=3, alpha=0
            )  # Invalid alpha
    ...
    def test_pocock_boundary(self):
        ...
        assert result["z_boundaries"][0] == pytest.approx(
            result["z_boundaries"][1], abs=1e-10
        )
        assert result["z_boundaries"][1] == pytest.approx(
            result["z_boundaries"][2], abs=1e-10
        )
    ...
    def test_conditional_power(self):
        ...
        # Test with default parameters
        result = stats_analysis.conditional_power(
            current_z=1.5, information_fraction=0.5
        )
    ...
    def test_optional_stopping_correction(self):
        ...
        assert result_sidak["adjusted_p_value"] == pytest.approx(
            1.0 - (1.0 - 0.04) ** 5, abs=1e-10
        )
        ...
        result_large_p = stats_analysis.optional_stopping_correction(
            p_value=0.3, num_looks=5
        )
    ...
    def test_log_likelihood_ratio_test(self):
        ...
        result = stats_analysis.log_likelihood_ratio_test(
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.log_likelihood_ratio_test(
                "not a float", model2_loglik, df1, df2
            )
    ...
    def test_model_selection_criteria(self):
        ...
        with pytest.raises(InvalidParameterError):
            stats_analysis.model_selection_criteria(
                loglik, 0, sample_size
            )  # df must be >= 1

        with pytest.raises(InvalidParameterError):
            stats_analysis.model_selection_criteria(
                loglik, df, df
            )  # sample_size must be > df
