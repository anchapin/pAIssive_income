...
class MockEmailAPI(MockExternalAPIBase):
    ...
    def send_email(
        ...
        return {
            "id": email["id"],
            "status": email["status"],
            "message": "Email sent successfully" if email["status"] == "sent" else "Failed to send email"
        }
    ...
    def get_sent_emails(self, to_email: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if to_email:
            return [
                email for email in self.sent_emails if (
                    email["to_email"] == to_email or
                    (isinstance(email["to_email"], list) and to_email in email["to_email"])
                )
            ]
...
class MockStorageAPI(MockExternalAPIBase):
    ...
    def list_buckets(self) -> List[Dict[str, Any]]:
        ...
        return [{"name": name, "objects_count": len(objects)} for name, objects in self.buckets.items()]

    def upload_object(self, data: bytes, key: str, bucket_name: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def list_objects(self, bucket_name: Optional[str] = None, prefix: Optional[str] = None) -> List[Dict[str, Any]]:
        """
