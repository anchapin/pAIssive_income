...
class TestNamespaceClearing(unittest.TestCase):
    ...
    def test_clear_specific_namespace(self):
        ...
        for key in all_keys_after:
            logger.debug(f"Key after clearing: {key}, Value: {self.cache_service.cache_manager.backend.get(key)}")
    ...
    def test_cache_manager_clear_namespace(self):
        ...
        def mock_parse_cache_key(key_str):
            if key_str == 'namespace1:key1':
                ...
                return CacheKey(model_id='namespace1', operation='key1', input_hash='hash', parameters_hash='hash')
            elif key_str == 'namespace1:key2':
                from ai_models.caching.cache_key import CacheKey
                return CacheKey(model_id='namespace1', operation='key2', input_hash='hash', parameters_hash='hash')
            else:
                return original_parse_cache_key(key_str)

        # Create a patch for the parse_cache_key function
        with patch('ai_models.caching.cache_manager.parse_cache_key', side_effect=mock_parse_cache_key):
            cache_manager = CacheManager()

            # Mock the backend methods
            cache_manager.backend.get_keys = MagicMock(return_value=['namespace1:key1', 'namespace1:key2'])
