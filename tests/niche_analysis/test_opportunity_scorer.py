...
def test_score_problem_severity():
    ...
    assert scorer._score_problem_severity(high_severity_problems) > scorer._score_problem_severity(medium_severity_problems)
    assert scorer._score_problem_severity(medium_severity_problems) > scorer._score_problem_severity(low_severity_problems)
    
    # Test mixed severity
    mixed_score = scorer._score_problem_severity(mixed_severity_problems)
    assert scorer._score_problem_severity(low_severity_problems) < mixed_score < scorer._score_problem_severity(high_severity_problems)
...
def test_score_monetization_potential():
    ...
    # Test different niches
    assert 0 <= scorer._score_monetization_potential("inventory management", market_data, problems) <= 1
    assert 0 <= scorer._score_monetization_potential("product description generation", market_data, problems) <= 1
    ...
    opportunities = [
        {
            ...
            "market_data": {"market_size": "large", "growth_rate": "high", "competition": "low"},
        ...
        },
        {
            ...
            "market_data": {"market_size": "medium", "growth_rate": "medium", "competition": "medium"},
        ...
        },
        {
            ...
            "market_data": {"market_size": "small", "growth_rate": "low", "competition": "high"},
