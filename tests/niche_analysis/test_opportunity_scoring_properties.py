...
def calculate_opportunity_score(factors, weights):
    """
...
class TestOpportunityScoringAlgorithmProperties:
    ...
    @given(
        ...
        weights1=factor_weights_strategy(),
        weights2=factor_weights_strategy(),
    )
    def test_weight_influence(self, factors, weights1, weights2):
        ...
        # Property: Different weights can lead to different scores for the same factors
        # (We're not asserting any specific relationship since weights could vary in complex ways)
        # This mainly tests that the scoring function actually uses the weights

        # Find the factor with the highest value
        max_factor = max(factors.items(), key=lambda x: x[1])[0]

        # If the weight for the highest factor is higher in weights1, score1 should be higher
        # (assuming a significant difference in weights)
        if weights1[max_factor] > weights2[max_factor] + 0.2:
