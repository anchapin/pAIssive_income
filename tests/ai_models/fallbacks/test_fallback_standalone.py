...
class FallbackManagerMock:
    ...
    def find_fallback_model(
            ...
            try:
                ...
                if strategy == FallbackStrategy.DEFAULT and self.default_model_id:
                    try:
                        ...
                        event = self._create_event(original_model_id, fallback_model, strategy, agent_type, task_type)
        ...
        if strategy == FallbackStrategy.NONE:
            ...
            if original_model_info and hasattr(original_model_info, "capabilities") and original_model_info.capabilities:
                ...
                for model in candidates:
                    if hasattr(model, "capabilities") and model.capabilities:
                        if any(cap in model.capabilities for cap in original_model_info.capabilities):
                            event = self._create_event(original_model_id, model, strategy, agent_type, task_type)
                            self.track_fallback_event(event)
                            return model, event
            ...
            if agent_type and agent_type in self.fallback_preferences:
                for model_type in self.fallback_preferences[agent_type]:
                    ...
                    if models:
                        event = self._create_event(original_model_id, models[0], strategy, agent_type, task_type)
                        self.track_fallback_event(event)
            ...
            all_models = self.model_manager.get_all_models()
            if all_models:
                event = self._create_event(original_model_id, all_models[0], strategy, agent_type, task_type)
                self.track_fallback_event(event)
            ...
            if original_model_info:
                ...
                if original_size > 0:
                    ...
                    if smaller_models:
                        event = self._create_event(original_model_id, smaller_models[-1], strategy, agent_type, task_type)
                        self.track_fallback_event(event)
                ...
                same_type_models = [m for m in candidates if m.type == original_model_info.type]
                if same_type_models:
                    event = self._create_event(original_model_id, same_type_models[0], strategy, agent_type, task_type)
                    self.track_fallback_event(event)
            ...
            if required_capabilities:
                ...
                if capable_models:
                    event = self._create_event(original_model_id, capable_models[0], strategy, agent_type, task_type)
                    self.track_fallback_event(event)
...
class TestFallbackManager(unittest.TestCase):
    ...
    def test_fallback_with_unsuccessful_result(self):
        ...
        metrics = fallback_manager.get_fallback_metrics()
        self.assertEqual(metrics[FallbackStrategy.DEFAULT.value]["total_count"], 2)  # One from find_fallback_model and one from track_fallback_event
        self.assertEqual(metrics[FallbackStrategy.DEFAULT.value]["success_count"], 1)  # From the initial call which is successful by default
