...
def test_version_compatibility_explicit():
    ...
    v1 = ModelVersion(
        version="1.0.0", model_id="test-model", is_compatible_with=["2.0.0"]
    )
    v2 = ModelVersion(version="2.0.0", model_id="test-model")
...
def test_registry_compatibility_check(version_registry, model_info):
    ...
    v1 = ModelVersion(
        version="1.0.0", model_id=model_info.id, is_compatible_with=["2.0.0"]
    )
...
def test_version_hash_verification(version_registry, tmp_path):
    ...
    version = version_registry.create_version_from_model(
        model_info=model_info, version_str="1.0.0"
    )
...
def test_concurrent_version_updates(version_registry, model_info):
    ...
    base_version = ModelVersion(
        version="1.0.0", model_id=model_info.id, features=["base-feature"]
    )
