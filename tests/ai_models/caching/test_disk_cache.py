...
def test_serialization_formats(temp_cache_dir):
    ...
    json_cache = DiskCache(
        cache_dir=os.path.join(temp_cache_dir, "json"),
        serialization="json"
    )
    
    # Test pickle serialization
    pickle_cache = DiskCache(
        cache_dir=os.path.join(temp_cache_dir, "pickle"),
...
def test_eviction_policies(temp_cache_dir):
    ...
    test_cases = [
        ...
        ("lfu", lambda cache: [cache.get("key1") for _ in range(3)]),  # Make key1 most frequently used
