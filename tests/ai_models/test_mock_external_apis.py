...
from tests.mocks.mock_huggingface_hub import mock_huggingface_hub, MockRepoInfo, HfHubHTTPError
...
class TestMockExternalAPIs(unittest.TestCase):
    ...
    def test_mock_openai_api(self):
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["choices"][0]["message"]["content"], "Hello! How can I help you today?")
    ...
    def test_mock_lmstudio_api(self):
        ...
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["choices"][0]["message"]["content"], "This is a response from LM Studio")
    ...
    def test_mock_huggingface_hub(self):
        ...
        repo_path = mock_huggingface_hub.snapshot_download(
            repo_id="gpt2"
        )
...
def test_with_pytest_fixtures(mock_http, mock_hf_hub):
    ...
    file_path = mock_hf_hub.hf_hub_download(
        repo_id="gpt2",
        filename="config.json"
    )
