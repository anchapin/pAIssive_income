...
class ProjectSharing:
    ...
    def share_project_with_user(self, 
                               project_id: str, 
                               user_id: str,
                               permission: SharingPermission,
                               shared_by: str,
                               workspace_id: Optional[str] = None,
                               expiry: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        for share_info in self.shared_projects.values():
            if (share_info["project_id"] == project_id and 
                share_info["shared_with_user"] == user_id and
                not share_info.get("revoked", False)):
        ...
        logger.info(f"Shared project {project_id} with user {user_id} (permission: {permission.value})")
        return share_info
    
    def share_project_with_workspace(self,
                                    project_id: str,
        ...
        for share_info in self.shared_projects.values():
            if (share_info["project_id"] == project_id and 
                share_info.get("shared_with_workspace") == target_workspace_id and
                not share_info.get("revoked", False)):
                raise SharingError(f"Project {project_id} is already shared with workspace {target_workspace_id}")
        ...
        logger.info(f"Shared project {project_id} with workspace {target_workspace_id} (permission: {permission.value})")
        return share_info
    ...
    def create_sharing_link(self,
        ...
        logger.info(f"Created sharing link for project {project_id} (permission: {permission.value})")
        return link_info
    ...
    def get_user_shared_projects(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for share_info in self.shared_projects.values():
            if (share_info.get("shared_with_user") == user_id and 
                not share_info.get("revoked", False)):
    ...
    def get_workspace_shared_projects(self, workspace_id: str) -> List[Dict[str, Any]]:
        ...
        for share_info in self.shared_projects.values():
            if (share_info.get("shared_with_workspace") == workspace_id and 
                not share_info.get("revoked", False)):
