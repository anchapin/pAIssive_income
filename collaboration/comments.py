...
class CommentSystem:
    ...
    def add_comment(self, 
        ...
        if resource_key not in self.resource_comments:
            self.resource_comments[resource_key] = {
                "top_level": [],
                "replies": {}
            }
    ...
    def delete_comment(self, comment_id: str, user_id: str) -> bool:
        ...
        if resource_key in self.resource_comments:
            if parent_id:
                if parent_id in self.resource_comments[resource_key]["replies"]:
                    if comment_id in self.resource_comments[resource_key]["replies"][parent_id]:
                        self.resource_comments[resource_key]["replies"][parent_id].remove(comment_id)
            else:
        ...
        if comment_user_id in self.user_comments and comment_id in self.user_comments[comment_user_id]:
    ...
    def get_resource_comments(self, 
                            resource_type: str,
                            resource_id: str,
        ...
        for comment_id in self.resource_comments[resource_key]["top_level"]:
            ...
            if comment:
                ...
                if include_replies and comment_id in self.resource_comments[resource_key]["replies"]:
    ...
    def add_reaction(self, 
        ...
        logger.info(f"Added {reaction_type.value} reaction to comment {comment_id} by user {user_id}")
        return reaction
