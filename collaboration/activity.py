...
class ActivityTracker:
    ...
    def get_workspace_activities(self, 
                               workspace_id: str,
                               limit: Optional[int] = None,
                               activity_types: Optional[List[ActivityType]] = None) -> List[ActivityLog]:
        """
    ...
    def search_activities(self, 
                         ...
                         user_id: Optional[str] = None,
                         workspace_id: Optional[str] = None,
                         project_id: Optional[str] = None) -> List[ActivityLog]:
        """
        ...
        for activity_data in reversed(list(self.activities.values())):  # Newest first
            ...
            if (query in (activity.description or "").lower() or
                any(query in str(value).lower() for value in activity.metadata.values())):
                activities.append(activity)
                if limit and len(activities) >= limit:
                    break
...
class NotificationManager:
    ...
    def mark_all_as_read(self, user_id: str) -> int:
        ...
        for notification_id in self.user_notifications[user_id]:
            if notification_id in self.notifications and not self.notifications[notification_id]["read"]:
                self.notifications[notification_id]["read"] = True
                self.notifications[notification_id]["read_at"] = datetime.now().isoformat()
                count += 1
    ...
    def create_activity_notification(self, 
                                   activity: ActivityLog,
                                   user_ids: List[str],
