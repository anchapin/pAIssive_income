...
def run_command(command: List[str], env: Optional[Dict[str, str]] = None) -> int:
    ...
    try:
        ...
        if os.path.exists(venv_python):
            ...
            if command[0] in ["flake8", "black", "isort", "mypy", "pytest", "bandit", "safety", "semgrep", "pylint"]:
...
def run_lint_checks() -> bool:
    ...
    # Check results
    all_passed = all(result == 0 for result in [flake8_result, black_result, isort_result, mypy_result])
...
def run_security_scan() -> bool:
    ...
    try:
        ...
        print_warning("Failed to run bandit with pyproject.toml config, falling back to default config...")
