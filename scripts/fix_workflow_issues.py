        ...
        for job_name, job_config in workflow["jobs"].items():
            ...
            for step in job_config["steps"]:
                ...
                if (
                    ...
                    and "mcp" in run_content.lower()
                    and "runner.os" in str(job_config)
                    and "Windows" in str(job_config)
                    and "--ignore" not in run_content
                ):
                    step["run"] = run_content.replace(
                        "pytest", "pytest --ignore=tests/test_mcp_top_level_import.py"
                    )
                    modified = True
    ...
    if "jobs" in workflow:
        for job_name, job_config in workflow["jobs"].items():
            ...
            if (
                ...
                and "fail-fast" not in job_config["strategy"]
            ):
