...
def increment_counter(name: str, value: int = 1, labels: Optional[Dict[str, 
    str]] = None):
    """
...
def start_timer(name: str, labels: Optional[Dict[str, str]] = None) -> Callable[[], 
    float]:
    """
    ...
    def stop_timer() -> float:
        ...
        with _metrics_lock:
            ...
            if len(_metrics["histograms"][f"{key}_histogram"]) > 1000:
                _metrics["histograms"][f"{key}_histogram"] = _metrics["histograms"][
                    f"{key}_histogram"
                ][-1000:]
        return elapsed
