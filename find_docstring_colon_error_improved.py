        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            for i, line in enumerate(lines):
                ...
                if re.search(r'""".*?"""\s*:', line) or re.search(r"'''.*?'''\s*:", line):
                    # Skip the file that's searching for patterns
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if file.endswith(".py"):
                ...
                # Skip virtual environment files and the search scripts
                if ".venv" not in file_path and "venv" not in file_path and "find_docstring_colon" not in file_path:
