...
class OpportunityScorer:
    ...
    def compare_opportunities(self, opportunities: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        recommendations = self._generate_comparison_recommendations(sorted_opportunities, comparative_analysis)
        ...
        # Cache the result
        default_cache.set(cache_key, result, ttl=self.cache_ttl, namespace="opportunity_comparisons")
    ...
    def _score_problem_severity(self, problems: List[Dict[str, Any]]) -> float:
        ...
        total_severity = sum(severity_scores.get(p.get("severity", "low").lower(), 0.0) for p in problems)
    ...
    def _score_solution_feasibility(self, niche: str, problems: List[Dict[str, Any]]) -> float:
        ...
        text_content_niches = ["content", "writing", "translation", "summarization", 
                               "description", "copywriting", "editing"]
        
        # Data processing niches are also more feasible
        data_niches = ["analytics", "analysis", "reporting", "monitoring", 
    ...
    def _score_monetization_potential(self, niche: str, market_data: Dict[str, Any], problems: List[Dict[str, Any]]) -> float:
        ...
        business_keywords = ["business", "enterprise", "professional", "b2b", 
    ...
    def _generate_assessment_and_recommendations(self, score: float, factor_scores: Dict[str, FactorScoreSchema], niche: str) -> tuple:
        """
        ...
        if "monetization_potential" in strengths:
            recommendations.append("Develop a premium pricing strategy based on high willingness to pay")
            
        # Add recommendations based on weaknesses
        if "market_size" in weaknesses:
            recommendations.append("Focus on a specific sub-segment of the market to maximize impact")
    ...
    def _calculate_score_distribution(self, opportunities: List[Dict[str, Any]]) -> ScoreDistributionSchema:
        """
    ...
    def _generate_comparison_recommendations(self, opportunities: List[Dict[str, Any]], analysis: ComparativeAnalysisSchema) -> List[str]:
        """
