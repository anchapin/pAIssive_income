...
def generate_summary_report() -> None:
    ...
    log("SUCCESS METRICS")
    log("-" * 15)
    log(f"✅ Improved workflow validity from ~25% to {len(valid_files)}/{len(valid_files) + len(invalid_files)} ({len(valid_files)/(len(valid_files) + len(invalid_files))*100:.1f}%)")
